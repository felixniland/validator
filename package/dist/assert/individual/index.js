...
export function assertNonNullable(v) {
    if (!IsIndividual.isNonNullable(v))
        throw new Error(`v is nullable: ${v}`);
}
function getExpectedMsg(iden) {
    return `expected ${VAL_IDEN_TO_PRETTY_MAP[iden]}`;
}
function getStdAsserter(type) {
